...
UNITS_PER_EM = 1000
FULLWIDTH_METRICS = (1000, 100)
HALFWIDTH_METRICS = (500, 63)
SPACE_METRICS = (500, 0)
...
def _load_source_font() -> TTFont:
    ...
    source = TTFont(ADOBE_NOTDEF_FILE, recalcTimestamp=False, lazy=False)
    try:
        units_per_em = source["head"].unitsPerEm
        metrics = source["hmtx"][".notdef"]
    except KeyError as error:
        source.close()
        raise ValueError(f"Invalid Adobe NotDef source: missing {error}") from error
    if units_per_em != UNITS_PER_EM or metrics != FULLWIDTH_METRICS:
        source.close()
        raise ValueError(
            "Unexpected Adobe NotDef metrics: "
            f"UPM {units_per_em}, hmtx {metrics}; expected "
            f"UPM {UNITS_PER_EM}, hmtx {FULLWIDTH_METRICS}"
        )
    return source
...
def _load_halfwidth_notdef() -> Glyph:
    ...
    glyph = pen.glyph()
    left_side_bearing = min(x for x, _ in glyph.coordinates)
    if (
        metadata.width * 2 != source_units_per_em
        or left_side_bearing != HALFWIDTH_METRICS[1]
    ):
        raise ValueError(
            "Unexpected Last Resort .notdef metrics: "
            f"UPM {source_units_per_em}, advance {metadata.width}, "
            f"scaled LSB {left_side_bearing}; expected the advance to be half "
            f"the UPM and the scaled LSB to be {HALFWIDTH_METRICS[1]}"
        )
    return glyph
...
def build_font() -> bytes:
    ...
    builder.setupHorizontalMetrics({".notdef": FULLWIDTH_METRICS})
    builder.setupCommonTables(FONT_NAME, POSTSCRIPT_NAME, UNITS_PER_EM, True)
...
def build_mono_font() -> bytes:
    ...
    halfwidth_glyph = _load_halfwidth_notdef()
    ...
    builder.setupHorizontalMetrics(
        {
            ".notdef": FULLWIDTH_METRICS,
            "tofu.half": HALFWIDTH_METRICS,
            "space": SPACE_METRICS,
        }
    )
    groups = _halfwidth_ranges()
    builder.setupFormat13CharacterMap(groups)
    builder.setupCommonTables(
        MONO_FONT_NAME, MONO_POSTSCRIPT_NAME, HALFWIDTH_METRICS[0], False
    )
